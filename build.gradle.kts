...
val dot: Configuration by configurations.creating
...
dependencies {
    ...
    dot("libre.doge.mps:dot:" + Versions.dot)
}
...
val extractDot by tasks.registering(Copy::class) {
    from({ dot.resolve().map { zipTree(it) } })
    into(dependencyDir)
}
...
publishing {
    repositories {
        if(branch.equals("main") || branch.startsWith("maintenance") || branch.startsWith("legacy")) {
            ...
            maven {
                name = "GitHubPackages"
                url = uri("https://maven.pkg.github.com/nkoester/mps-dependencyDiagram")
                credentials {
